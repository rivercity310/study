def print_nutritions():
    ...
    if BMI > 25:
        print("BMI {0:.2f}은(는) <비만>에 해당합니다.".format(BMI))
        print("식습관을 개선하고 유산소 운동과 근력운동을 겸해야합니다.")
    elif BMI > 23:
        print("BMI {0:.2f}은(는) <과체중>에 해당합니다.".format(BMI))
        print("조금만 더 체중을 감량해보세요!")
    elif BMI > 18.5:
        print("BMI {0:.2f}은(는) <정상>에 해당합니다.".format(BMI))
        print("잘하고있어요!")
    else:
        print("BMI {0:.2f}은(는) <저체중>에 해당합니다.".format(BMI))
        print("체중을 조금 늘려보세요!")
    ...
    if weight_interval >= 0:
        print("BMI {0:.2f}이(가) 되려면 <{1:.2f}kg 감량> 해야해요!".format(
            goal_BMI, weight_interval))
    else:
        print("BMI {0:.2f}이(가) 되려면 <{1:.2f}kg 증량> 해야해요!".format(
            goal_BMI, -weight_interval))
