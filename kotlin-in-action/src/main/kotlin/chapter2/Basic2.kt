...
private enum class Color (private val r: Int, private val g: Int, private val b: Int) {
...
private fun mix(color1: Color, color2: Color) = when(setOf(color1, color2)) {       // when 식의 인자로 아무 객체나 사용 가능 !!
...
private fun mix2(color1: Color, color2: Color) = when {
...
private interface Expr                          // 아무 메서드도 선언하지 않고 단지 여러 타입의 식 객체를 아우르는 공통 타입 역할만 수행
private class Num(val value: Int): Expr         // value라는 프로퍼티만 존재하는 단순 클래스로 Expr 인터페이스를 구현한다
private class Sum(val left: Expr, val right: Expr): Expr       // Expr 타입의 객체라면 어떤 것이나 Sum 연산의 인자 (Num이나 Sum이 인자로 올 수 있다)
private class Mul(val left: Expr, val right: Expr): Expr

/* java style code */
private fun evalJavaStyle(e: Expr): Int {
...
private fun evalMul(e: Expr): Int {
    if (e is Num) return e.value
    else if (e is Mul) return evalMul(e.left) * evalMul(e.right)
    else throw IllegalArgumentException("Unknown Expression")
}
...
fun basic2_ex1() {
    println(evalMul(Mul(Num(2), Num(5))))
}