...
public:
	...
	unsigned find(unsigned x) {
		vector<Node>::iterator node_it = find_if(nodes.begin(), nodes.end(), [x](Node n) { return n.id == x; });
...
// 트리도 그래프의 한 종류(사이클이 없는) 이므로 최소 신장 트리도 Graph 객체로 반환
template <typename T>
Graph<T> minimum_spanning_tree(const Graph<T>& G) {
	
	// 에지 가중치를 이용한 최소 힙을 구성
	...
	// 정점 개수에 해당하는 크기의 디스조인트-셋 생성 및 초기화
	...
	while (!edge_min_heap.empty()) {
		...
		// 선택한 엣지가 사이클을 생성하지 않으면 해당 에지를 MST에 추가
